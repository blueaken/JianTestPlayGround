...
//            }
            ...
            appendLeftList.add(5);
            resultLeftList = addMapData(resultLeftList, 5);
            appendLeftList.add(4);
            resultLeftList = addMapData(resultLeftList, 4);

            appendRightList.add(1);
            resultRightList = addMapData(resultRightList, 1);
            appendRightList.add(5);
            resultRightList = addMapData(resultRightList, 5);
            appendRightList.add(1);
            resultRightList = addMapData(resultRightList, 1);
            appendRightList.add(3);
            resultRightList = addMapData(resultRightList, 3);
            appendRightList.add(2);
            resultRightList = addMapData(resultRightList, 2);


            final ThreadSafeLinkedList_2471 list = new ThreadSafeLinkedList_2471();
...
//                printStream.write("accepted".getBytes("Utf-8"));
                ...
                System.out.println("Current List: " + ListNode.getListString(list.getLinkedList()));
            } else {
//                printStream.write(ListNode.getListString(list.getLinkedList()).getBytes("Utf-8"));
                System.out.print("Warning, uneven list operation.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            try{
