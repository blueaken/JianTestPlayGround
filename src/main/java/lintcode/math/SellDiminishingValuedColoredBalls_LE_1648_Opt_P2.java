...
public class SellDiminishingValuedColoredBalls_LE_1648_Opt_P2 {
    /*
     gut feeling - greedy with the help of max heap, Time - O(NlogM), N - order number, M - color kinds
        ...
        - Ref 花花酱讲解 https://www.youtube.com/watch?v=kBxqAnWo9Wo，使用等差数列公式辅助提高性能,
        - Time - O(NLogN) + O(1) = O(NLogN) - sorting
    ...
    public int maxProfit(int[] inventory, int orders) {
        ...
        while (orders > 0) {
            ...
            if (color * tran_n <= orders) {
                ...
                tran_n = orders / color;
                int reminder = orders % color;
                long nxt_p = cur - tran_n;
                sum = ((cur + nxt_p + 1) * tran_n / 2 * color + reminder * nxt_p) % mod;
            }
