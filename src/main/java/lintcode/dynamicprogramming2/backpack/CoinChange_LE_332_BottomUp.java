...
public class CoinChange_LE_332_BottomUp {
    /*
        10.16.2022
    ...
    */
    ...
    public int coinChange(int[] coins, int amount) {
        ...
        //答案最多为amount, 初始值设为amount + 1相当于正无穷；为啥不直接用Integer.MAX_VALUE? 后面公式中有dp[i-coin] + 1, 会导致overflow

        //base case
        dp[0] = 0; //amount为0时，需要0个硬币

        //dp
        for (int i = 1; i <= amount; i++) { //外层 for 循环在遍历所有状态1的所有取值
            for (int coin : coins) { //内层 for 循环在遍历所有状态2的所有取值
                if (i - coin < 0) { //子问题无解，则跳过
                    continue;
                }
